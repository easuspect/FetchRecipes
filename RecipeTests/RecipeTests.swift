...
//  RecipeTests
//
//  Created by Tolga Telseren on 12/15/24.
//
...
final class RecipeTests: XCTestCase {
    ...
    func testRecipesCount() async throws {
        let recipes = try NetworkManager.parseData(data: json.data(using: .utf8)!)
        XCTAssertEqual(recipes.count, 63)
    }

    func testRecipesContainsAppleAndBlackberryCrumble() async throws {
        let recipes = try NetworkManager.parseData(data: json.data(using: .utf8)!)
        let contains = recipes.contains(where: { $0.name == "Apple & Blackberry Crumble" })
        XCTAssertTrue(contains)
    }
    
    func testRecipesContainsBattenbergCake() async throws {
        let recipes = try NetworkManager.parseData(data: json.data(using: .utf8)!)
        XCTAssertTrue(recipes.contains(where: { $0.name == "Battenberg Cake" }))
    }
    
    func testBritishCuisineCount() async throws {
        let recipes = try NetworkManager.parseData(data: json.data(using: .utf8)!)
        XCTAssertEqual(recipes.filter { $0.cuisine == "British" }.count, 27)
    }
